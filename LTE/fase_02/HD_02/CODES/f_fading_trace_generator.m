function f_fading_trace_generator(multipathProfile, v_km_h)
% Inputs
%    v_km_h: speed of mobile node (doppler spread model)
%
disp(' ')
disp('Generating files for ');
% Test available speeds
switch v_km_h
    case 0
        disp('For speed of 0 kmph')
    case 3
        disp('For speed of 3 kmph')
    case 30
        disp('For speed of 30 kmph')
    case 60
        disp('For speed of 60 kmph')
    otherwise
        disp('Parameter setting problem: valid inputs are 0, 3, 30 or 60 kmph.')
end

switch lower(multipathProfile)
    case 'epa'
        chTitle = ['Fading Pedestrian for speed of '  num2str(v_km_h) ' kmph' ];
        disp('EPA multipath profile')
        delays = [0 30e-9 70e-9 90e-9 120e-9 190e-9 410e-9];
        power = [0.0 -1.0 -2.0 -3.0 -8.0 -17.2 -20.8];
    case 'eva'
        chTitle = ['Fading Vehicular EVA for speed of '  num2str(v_km_h) ' kmph' ];
        delays = [0 30e-9 150e-9 310e-9 370e-9 710e-9 1090e-9 1730e-9 2510e-9];
        power = [0.0 -1.5 -1.4 -3.6 -0.6 -9.1 -7.0 -12.0 -16.9];
        disp('EVA multipath profile')
    case 'etu'
        chTitle = ['Fading Urban ETU for speed of '  num2str(v_km_h) ' kmph' ];
        disp('ETU multipath profile')
        delays = [0 50e-9 120e-9 200e-9 230e-9 500e-9 1600e-9 2300e-9 5000e-9];
        power = [-1.0 -1.0 -1.0 0.0 0.0 0.0 -3.0 -5.0 -7.0];
    otherwise
        disp('Parameter setting problem: valid inputs are ETU, EVA and EPA.')
end
...
tag = [multipathProfile '_' num2str(v_km_h)  ];
title(chTitle)
...
saveas(gcf, strcat('fading_trace_',tag,'.png'))
saveas(gcf, strcat('fading_trace_',tag,'.fig'))

axis([0 100 0 100 -50 10])
saveas(gcf, strcat('fading_trace_zoom_',tag,'.png'))
saveas(gcf, strcat('fading_trace_zoom_',tag,'.fig'))
